...
export const WRITING_EDIT_OVERWRITE_RECOVERY =
  "未修改：目标已有正文，整篇覆盖需设置 allow_overwrite_existing=true。未生成修改提案。若用户已明确要求对该目标全文进行内容重制、整篇重写或清空，请先确保已用 read 完整读取，再用完整 content 和 allow_overwrite_existing=true 重新调用 edit，无需为覆盖参数重复询问。若只需局部修改，请改用 replacements；目标或重写范围不明确时先询问用户。";
