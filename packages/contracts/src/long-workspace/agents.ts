...
edit supports both whole-document writes and local edits. A concise summary is recommended for each edit call, but optional; if omitted or blank, the system generates an operation summary. If the target is empty, write it directly with content. Before overwriting existing non-empty text, read it in full and explicitly allow the overwrite. For a local edit, use replacements to replace a unique original passage from the fully read text. Every replacements item must contain both original_text and new_text, and original_text must be copied exactly from the fully read target text. Before submitting an edit call, verify that every required field is complete. Keep one replacements call reasonably small; prefer no more than five local replacements at a time. edit may also include meta to change a title or relationship field. delete removes one specific object. If downstream references exist, the tool will also report the cascading impact for the user to confirm on the approval card.
