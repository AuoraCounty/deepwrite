...
describe("prompt attachments", () => {
  it("extracts pasted files without treating clipboard text as an attachment", () => {
    ...
    expect(promptAttachmentFilesFromClipboard(clipboardData)).toEqual([image]);
  });

  it("falls back to the clipboard file list when item access is unavailable", () => {
    const document = new File(["正文"], "pasted.txt", { type: "text/plain" });
    const clipboardData = {
